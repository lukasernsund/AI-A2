...
myFunction <- function(moveInfo, readings, positions, edges, probs) {
    ...
    transition = create_transition_matrix()
    emission = create_emission_matrix(readings, probs)

    #calculate newState
    newState = create_state_prob_vector(moveInfo$mem$prevState,
                                        transition,
                                        emission,
                                        positions)

    #get max index of newState
    goal = which.max(newState)

    #if goal is in neighboring node, ge there
    ...
    #find path
    path = bfSearch(positions[3], goal, edges)

    #if croc is two nodes away
    if (length(path) >= 2){
        moveInfo$moves = c(path[1], path[2])
    }

    #if croc is one node away
    if (length(path) == 1){
        moveInfo$moves = c(path[1], 0)
    }

    #if croc is here, search
    if(length(path) == 0){
        moveInfo$moves = c(0,0)  
    }

    #update prevState and status
